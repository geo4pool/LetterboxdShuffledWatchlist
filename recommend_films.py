...
def recommend_films(username):
    ...
    distances, indices = knn.kneighbors(ratings_matrix.values, n_neighbors=10)
    similar_dict = {}
    for i, similar in enumerate(indices):
        id_i = ratings_matrix.iloc[i].name
        # print(id_i, id_name_dict[id_i], "similar films are:::::")
        # for x in similar:
        # id_x = ratings_matrix.iloc[x].name
        # print(id_name_dict[id_x])
    ...
    # favourites = get_favourites(username)
